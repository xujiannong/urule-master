...
(function(Handsontable){
	...
	URule.DecisionTable=function(id){
		...
		const buttons=`<nav class="navbar navbar-default" style="margin: 5px">
			<div>
				<div>
					...
					${saveButton}
					<div class="btn-group btn-group-sm navbar-btn" style="margin-left:5px;margin-top:0px;margin-bottom: 0px" role="group" aria-label="..."> 
					  <button id="importExcelButton" type="button" class="btn btn-default"><i class="glyphicon glyphicon-share-alt"> 导入Excel</i></button>
					</div>
		        </div>
			</div>
		</nav>`;
        ...
        $("#importExcelButton").click(function(){
            if(!self.importExcelDialog){
                self.importExcelDialog=new urule.ImportExcelDialog(self);
            }
            self.importExcelDialog.open();
        });
